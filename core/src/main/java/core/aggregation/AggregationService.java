...
public class AggregationService {
	...
	public void loadRepositoryStatements(String sparqlQuery, String[] sparqlRules /* SPIN Like*/) {
		...
		for(org.eclipse.rdf4j.model.Statement st : model) {
			...
			String ctxStr = st.getContext() != null ? st.getContext().stringValue() : null;
			Context context = null;
			if(ctxStr != null) {
				context = new ContextImpl(core.model.Resource.getResource(ctxStr));
			} else {
				for(org.eclipse.rdf4j.model.Statement statType : model.getStatements(st.getSubject(), RDF.TYPE, null, (Resource)null)) {
					String typeObj = statType.getObject().stringValue();
					if(typeObj.indexOf("rdf-") == -1) {
						context = new ContextImpl(core.model.Resource.getResource(typeObj));
						break;
					} else {
						context = new ContextImpl(core.model.Resource.getResource(RDFS.RESOURCE.stringValue()));
						break;
					}
				}
			}
			...
			for(org.eclipse.rdf4j.model.Statement statType : model.getStatements(st.getSubject(), RDF.TYPE, null, (Resource)null)) {
				...
				Context ctx = null;
				if(typeObj.indexOf("rdf-") == -1) {
					ctx = new ContextImpl(core.model.Resource.getResource(typeObj));
					ck.getInstances().add(ctx);
					ck.getAttributes(ctx).add(property);
					ck.getValues(ctx, property).add(object);
					break;
				} else {
					ctx = new ContextImpl(core.model.Resource.getResource(RDFS.RESOURCE.stringValue()));
					ck.getInstances().add(ctx);
					ck.getAttributes(ctx).add(property);
					ck.getValues(ctx, property).add(object);
					break;
